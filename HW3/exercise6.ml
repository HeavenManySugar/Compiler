...
let recognize (a: autom) (s: string) : bool =
  ...
  let rec aux (q: Cset.t) (i: int) : bool =
    if i == n then
      Cset.mem eof q
    else
      let c = s.[i] in
      let q' = Smap.find q a.trans in
      aux (Cmap.find c q') (i + 1)
    in
  try aux a.start 0 with Not_found -> false
...
let fprint_autom_ml fmt a =
  ...
  let print_state s tr =
    ...
    if !first then begin first := false; Format.fprintf fmt "let rec " end
    else Format.fprintf fmt "and ";
    Format.fprintf fmt "state%d b =@\n" (Smap.find s num);
    if Cset.mem eof s then Format.fprintf fmt "b.last <- b.current;@\n";
...
let next_char b =
  ...
  c
";
Format.fprintf fmt "%a@]@." fprint_autom_ml a;
  close_out ch
